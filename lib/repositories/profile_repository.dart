...
class ProfileRepository {
  ...
  Profile _fromJson(Map<String, dynamic> j) => Profile.fromJson(j);

  Future<Result<Profile>> getProfile() async {
    final res = await _api.get('/auth/me');
    return switch (res) {
      ResultSuccess(data: final d) => ResultSuccess<Profile>(
        _fromJson(d['data'] as Map<String, dynamic>),
      ),
  ...
  Future<Result<bool>> updateSettings({
    ...
    final res = await _api.put(
      '/users/settings',
      body: {
        'settings': {'notifications': notifications, 'language': language},
      },
    );
    return switch (res) {
      ResultSuccess() => const ResultSuccess<bool>(true),
    ...
    try {
      ...
      if (res.statusCode >= 200 &&
        ...
        return ResultSuccess<Profile>(
          _fromJson(body['data'] as Map<String, dynamic>),
        );
      }
      ...
      return FailureResult(
        NetworkFailure(message: msg, statusCode: res.statusCode),
      );
    } catch (e) {
      return FailureResult(
        NetworkFailure(message: 'Upload avatar thất bại: $e'),
      );
    }
  }
